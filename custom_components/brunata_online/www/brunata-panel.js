...
class BrunataOnlinePanel extends HTMLElement {
  ...
  _delta(meter) {
    return consumptionDelta(meter.history, this._settings.period);
  }
  ...
  _cost(value, medium, unit) {
    const price = unitPrice(this._settings, medium, unit);
  ...
  _sparkline(meter, color) {
    ...
    if (points.length < 2)
      return `<div class="no-chart">${this._t("noData")}</div>`;
    ...
    const coords = values
      ...
      .join(" ");
    return `<svg class="spark" viewBox="0 0 100 48" preserveAspectRatio="none" aria-label="${this._t(
      "history",
    )}">
  ...
  _overview(meters) {
    if (!meters.length) {
      return `<section class="card">${this._empty()}</section>`;
    }
    const groups = groupCompatibleMeters(meters)
      .map((selected) => {
        const medium = selected[0].medium;
        const [icon, color] = MEDIA[medium];
        const unit = selected[0].unit;
        const label = `${this._medium(medium)} (${unit})`;
        const deltas = selected
          .map((meter) => this._delta(meter))
        ...
        <strong>${this._format(delta, unit)}</strong><small>${this._t(
          this._settings.period === 7 ? "last7" : "last30",
        )}${
          this._cost(delta, medium, unit)
            ? ` · ${this._cost(delta, medium, unit)}`
            : ""
        }</small>
    ...
    return `<section class="metrics">${groups || this._empty()}</section>
      <section class="card"><div class="section-title"><div><h2>${this._t(
        "history",
      )}</h2><p>Brunata Online</p></div></div>
      <div class="reading-list">${meters
        .slice(0, 6)
        .map((meter) => this._reading(meter))
        .join("")}</div></section>`;
  }
  ...
  _reading(meter) {
    ...
    return `<div class="reading"><span class="reading-icon" style="color:${color}"><ha-icon icon="${icon}"></ha-icon></span>
      <span class="reading-name"><b>${escapeHtml(
        meter.name,
      )}</b><small>${label}${
        meter.reading_date
          ? ` · ${escapeHtml(
              formatReadingDate(meter.reading_date, getLocale(this._hass)),
            )}`
          : ""
      }</small></span>
  ...
  _consumption(meters) {
    return `<section class="card"><div class="section-title"><div><h2>${this._t(
      ...
      <div class="charts">${
        meters
          .map((meter) => {
            const [, color] = MEDIA[meter.medium] || MEDIA.other;
            const delta = this._delta(meter);
            return `<article class="chart"><header><div><b>${escapeHtml(
              meter.name,
            )}</b><small>${this._format(delta, meter.unit)}${
              this._cost(delta, meter.medium, meter.unit)
                ? ` · ${this._cost(delta, meter.medium, meter.unit)}`
                : ""
            }</small></div><strong>${this._format(
              meter.value,
              meter.unit,
            )}</strong></header>${this._sparkline(meter, color)}</article>`;
          })
          .join("") || this._empty()
      }</div></section>`;
  }
  ...
  _metersView(meters) {
    return `<section class="card"><div class="section-title"><div><h2>${this._t(
      "devices",
    )}</h2><p>${meters.length} ${this._t("devices")}</p></div></div>
      <div class="meter-grid">${
        meters
          .map((meter) => {
            const [icon, color] = MEDIA[meter.medium] || MEDIA.other;
            return `<article class="meter"><span class="meter-icon" style="--accent:${color}"><ha-icon icon="${icon}"></ha-icon></span><div><b>${escapeHtml(
              meter.name,
            )}</b><small>${this._medium(meter.medium)}${
              meter.number ? ` · ${escapeHtml(meter.number)}` : ""
            }</small></div><strong>${this._format(
              meter.value,
              meter.unit,
            )}</strong></article>`;
  ...
  _settingsView() {
    ...
    return `<section class="card settings-card"><div class="section-title"><h2>${this._t(
      "settings",
    )}</h2></div><form>
      <label><span>${this._t(
        "period",
      )}</span><select name="period"><option value="7" ${
        s.period === 7 ? "selected" : ""
      }>${this._t("last7")}</option><option value="30" ${
        s.period === 30 ? "selected" : ""
      }>${this._t("last30")}</option></select></label>
      <label><span>${this._t("precision")}</span><select name="precision">${[
        0, 1, 2, 3,
      ]
        .map(
          (value) =>
            `<option value="${value}" ${
              s.precision === value ? "selected" : ""
            }>${value}</option>`,
        )
        .join("")}</select></label>
      <label><span>${this._t(
        "currency",
      )}</span><input name="currency" maxlength="3" value="${escapeHtml(
        s.currency,
      )}"></label>
      <label><span>${this._medium("cold_water")} · ${this._t(
        "price",
      )} / m³</span><input name="waterPrice" type="number" min="0" step="0.01" value="${
        s.waterPrice
      }"></label>
  ...
  _empty() {
    return `<div class="empty" role="status"><ha-icon icon="mdi:gauge-empty"></ha-icon><b>${this._t(
      "noData",
    )}</b></div>`;
  }
  ...
  _render() {
    ...
    let body = `<div class="loading" role="status" aria-live="polite">${this._t(
      "loading",
    )}</div>`;
    if (this._error)
      body = `<div class="error" role="alert">${this._t(
        "unavailable",
      )}: ${escapeHtml(this._error)}</div>`;
    ...
    this.shadowRoot.innerHTML = `<style>${STYLES}</style><div class="page" lang="${escapeHtml(
      ...
      <header class="hero"><div class="brand"><img src="/brunata-online/icon.png" alt=""><div><h1>Brunata Online</h1><p>${this._t(
        "consumption",
      )}</p></div></div>
      <div class="actions">${
        accounts.length > 1
          ? `<select aria-label="Brunata Online">${accounts
              .map(
                (item, i) =>
                  `<option value="${i}" ${
                    i === this._account ? "selected" : ""
                  }>${escapeHtml(item.title)}</option>`,
              )
              .join("")}</select>`
          : ""
      }<button class="refresh" aria-label="${this._t(
        "refresh",
      )}" ${this._loading ? "disabled" : ""}><ha-icon icon="mdi:refresh"></ha-icon></button></div></header>
        ...
        .map(
          ([id, key]) =>
            `<button data-tab="${id}" class="${
              this._tab === id ? "active" : ""
            }" ${this._tab === id ? 'aria-current="page"' : ""}>${this._t(
              key,
            )}</button>`,
        )
        .join("")}</nav>
      <main>${
        account
          ? `<div class="status"><span class="dot ${
              account.available ? "ok" : "bad"
            }"></span>${
              account.available ? this._t("updated") : this._t("unavailable")
            }<span>${
              account.last_update
                ? new Date(account.last_update).toLocaleString(locale)
                : ""
            }</span></div>`
          : ""
      }${body}</main></div>`;

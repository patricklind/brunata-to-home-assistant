...
class BrunataOnlinePanel extends HTMLElement {
  ...
  _empty() {
    return `<div class="empty"><ha-icon icon="mdi:gauge-empty"></ha-icon><b>${this._t(
      "noData"
    )}</b></div>`;
  }
  ...
  _render() {
    ...
    if (this._error)
      body = `<div class="error">${this._t("unavailable")}: ${escapeHtml(
        this._error
      )}</div>`;
    ...
    this.shadowRoot.innerHTML = `<style>${STYLES}</style><div class="page" lang="${escapeHtml(
      ...
      "refresh"
    )}"><ha-icon icon="mdi:refresh"></ha-icon></button></div></header>
      <nav>${[
        ["overview", "overview"],
